...
checkpointpath = './model/sample/boston/boston_check-{epoch:02d}-{loss:.4f}.hdf5'
checkpoint = ModelCheckpoint(filepath = checkpointpath, monitor = 'loss', save_best_only = True, mode = 'auto')

model.compile(loss = 'mse', optimizer = 'adam', metrics = ['mse'])
model.fit(x_train, y_train, epochs = 30, batch_size = 10, callbacks = [checkpoint], validation_split = 0.2, verbose = 1)
