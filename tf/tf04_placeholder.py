...
# placeholder 준비
# 처음 보는 placeholder, free_dict로 값을 넣어 준다, 익숙해져야함!
...
print(sess.run(adder_node, feed_dict={a:[1,3], b:[2,4]})) # [3. 7.] # 리스트 형태도 가능하다 -> 이 말은 곧 넘파이 연산이 이루어진다는 말***

# add_and_triple이라는 또 다른  node 생성
add_and_triple = adder_node * 3
print(sess.run(add_and_triple, feed_dict = {a:3, b:4.5})) # 22.5

# 그래프로 표현했을 때 두 개의 노드가 합쳐져(a, b) -> adder_node가 출력되는 그런 느낌적인 느낌의 그래프
# 전체적으로 봤을 때 무언가가 떠오른다? -> 인공신경망 레이어 구성하는 것과 비슷
